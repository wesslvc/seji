...
const LD_SLIDES=[
 {act:'name',  ic:'globe', mc:'#6fa6e0', tt:'나라 이름 맞히기', ds:'지도에서 나라를 클릭하고 이름을 맞혀요. 3번 안에 맞히면 색이 칠해져요.', pts:'국가당 1점'},
 {act:'border', ic:'border',mc:'#5f9ad6', tt:'접경국 퀴즈', ds:'국경을 맞댄 이웃 나라로 추리하는 퀴즈. 난이도에 따라 방식이 달라져요.', pts:'1 · 3 · 9 · 40~60점', diff:'bdiff', levels:['L','M','H','X'],
  dd:{L:'하 · 지도에서 클릭해 맞히기 (1점)',M:'중 · 지도 없이 이름 입력 (3점)',H:'상 · 접한 나라 모두 쓰기 (비율별 2·5·9점)',X:'하드코어 · 4개국 이상과 접한 나라만. 개수도 안 알려 주고 건너뛰기도 없어요. 완벽해야 40점(아프리카 60점), 하나라도 어긋나면 놓친 곳 −2 · 잘못 적은 곳 −5만 남아요'}},
 {act:'religion',ic:'book', mc:'#e0bd94', tt:'종교 구성', ds:'원그래프를 보고 나라별 종교 구성을 맞혀요.', pts:'1 · 2 · 3점', diff:'rdiff',
  dd:{L:'하 · 상위 종교 70%+ 국가만 (1점)',M:'중 · 모든 국가 · 힌트 있음 (2점)',H:'상 · 3번 틀려야 공개 (3점)'}},
 {act:'river', ic:'wave',  mc:'#79b4e8', tt:'하천 맞히기', ds:'세계 주요 하천 60개를 경로·통과국으로 맞혀요.', pts:'3~10점', diff:'vdiff',
  dd:{L:'하 · 물줄기 모양 보고 이름 맞히기 (3점)',M:'중 · 지나는 나라 모두 클릭 (5점 만점)',H:'상 · 세계지도에 경로 그리기 (일치도×10점)'}},
 {act:'korea', ic:'pin',   mc:'#d9b489', tt:'한국지리', ds:'대한민국 행정구역 위치를 지도에서 맞혀요.', pts:'1점', diff:'kdiff', levels:['L','M'],
  dd:{L:'하 · 도·특별시·광역시 단위 (시·도 17개)',M:'중 · 전국 시·군 단위 전체'}},
 {act:'climate', ic:'climate', mc:'#87bcdf', tt:'기후 맞히기', ds:'기후 그래프 10개와 지도 위 지점 10개를 연결해요. 온대·냉한대·열대·전기후 라운드로 구성돼요.', pts:'2~8점', diff:'cldiff', levels:['L','M','H'],
  dd:{L:'하 · 지도 위 지점의 쾨펜 기후 기호 맞히기 (2점)',M:'중 · 17년간 평가원 출제 지역 (4점)',H:'상 · 전 세계 1000+ 지점 (8점)'}},
 {act:'trade',  ic:'trade', mc:'#8aa6cf', tt:'무역 구조', ds:'수출·수입 품목 트리맵을 보고 어느 나라인지 맞혀요.', pts:'2~9점', diff:'tdiff', tkind:true},
 {act:'tenergy',ic:'power', mc:'#dcb184', tt:'에너지 구성', ds:'발전·에너지 믹스를 보고 나라를 맞혀요. 유형 필터도 고를 수 있어요.', pts:'2 · 4 · 6점', diff:'ediff',
  dd:{L:'하 · 특징 뚜렷한 국가만 (2점)',M:'중 · 모든 국가 · 힌트 있음 (4점)',H:'상 · 힌트 없음 (6점)'}, esub:true},
 {act:'worlddict', ic:'dict', mc:'#d3bb92', tt:'세지 위키', ds:'국가별 수도·인구·GDP·기후·수출·에너지·종교·접경국 정보를 한눈에 봐요. 설명은 누구나 수정을 제안할 수 있어요.', pts:'모두가 만드는 사전', dict:true},
 {act:'stat', ic:'trade', mc:'#78b2ce', tt:'통계 순위 테스트', wide:true,
  tt2:'지도에서 1위부터 5위까지 순서대로',
  ds:'종교 신자 수 · 쌀·밀·옥수수의 생산/수출/소비/수입 · 소·양·돼지 사육두수 · 석유·석탄·천연가스의 생산/수출/수입. 28개 통계를 순위대로 클릭해요.',
  pts:'순위당 2점 (통계당 10점)', stcat:true},
 /* 3×3 배치를 흐트러뜨리지 않도록 맨 아래 가로 한 칸으로 붙인다 */
 {act:'suteuk', ic:'book', mc:'#cc9fa8', tt:'9모대비 수특퀴즈', wide:true,
  ds:'수특지엽 1~4강과 「이것이 수특 정리다」 특강 자료 01~25에 나온 내용만 물어봐요. 지도에 찍힌 지점의 기후 그래프 고르기, 순서 직접 입력, 지도 클릭 등 외운 대로 써야 풀리는 형태예요. 아래에서 주제·형식을 골라 그 부분만 집중해서 풀 수 있어요. 암기법은 힌트 버튼을 눌러야 보이고, 보면 그 문항 배점이 절반이 돼요. 틀린 문항은 오답노트 PDF로 저장할 수 있어요.',
  pts:'문항당 3점'}
];
const TRADE_DD={
...
function ldRestore(){
  ...
  if(!d)return null;
  if(['L','M','H','X'].includes(d.bdiff))LD.bdiff=d.bdiff;
...
function ldBuildGrid(){
  ...
  LD_SLIDES.forEach(sl=>{
    ...
    c.className='ld-cell'+(sl.disabled?' disabled':'')+(sl.wide?' wide':'');c.dataset.act=sl.act;c.style.setProperty('--mc',sl.mc);
    const pt=sl.disabled?'<span class="ld-soon">준비중</span>':sl.pts;
    const txt=sl.wide
      ?'<div class="ld-cell-txt"><div class="ld-cell-tt">'+sl.tt+'</div></div><div class="ld-cell-pt">'+pt+'</div>'
      :'<div class="ld-cell-tt">'+sl.tt+'</div><div class="ld-cell-pt">'+pt+'</div>';
...
function ldRenderDetail(){
  ...
  if(sl.diff){
    rows+='<div class="ld-dt-row" data-diff="'+sl.diff+'"><span class="ld-dt-lb">난이도</span>'
      +(sl.levels||['L','M','H']).map(d=>'<button type="button" class="ld-chip'+(LD[sl.diff]===d?' on':'')+'" data-d="'+d+'">'+({L:'하',M:'중',H:'상',X:'하드코어'})[d]+'</button>').join('')+'</div>'
...
function startFromLanding(){
  ...
  if(rawActs.includes('border')){
    ...
    rawActs.push((borderDiff==='H'||borderDiff==='X')?'rborder':'border');
  }
  ...
  if(acts.includes('rborder')&&borderDiff==='X')key+='_hard';
...
function switchTab(key){
  ...
  else if(suteukOn){ /* 수특퀴즈는 문항 종류에 따라 카드 화면과 지도 화면을 오간다 */
    mapMode='suteuk';
    try{dynMask.textContent='';}catch(e){} /* 다른 모드의 출제 범위 마스크가 남아 지도가 잠기는 걸 막는다 */
...
function rbqHardTaunt(){
  ...
  if(n>15&&n%5===0)return n+'번 연속이에요. 하부터 하고 오세요, 진심으로';
...
function repaintMap(){
  clearMapColors();
  const src=(mapMode==='border')?BQ.status:(mapMode==='rborder')?RBQ.status:(mapMode==='rvc'||mapMode==='rvd')?{}:S.status;
...
function bqShowCurrent(){
  if(document.getElementById('bq-end').classList.contains('on'))return; /* 완료창 표시 중엔 진행 갱신 중단 */
...
function bqEnd(){
  ...
  window._pendingReset=()=>resetBorderQuiz(true);
}
function openBQList(){
...
function rbqPool(){
  let base=S.activeSet?[...S.activeSet]:Object.keys(COUNTRIES);
  const min=RBQ.hard?RBQ_HARD_MIN:1;
  return base.filter(i=>BORDERS[i]&&BORDERS[i].length>=min);
}
function rbqInit(filterKey){
  RBQ.saveKey='rbq_'+(filterKey||'all');
  RBQ.noMap=true;
  /* 범위 키에 담아 두면 이어하기로 들어와도 하드코어인지 알 수 있다 */
  RBQ.hard=/_hard(_|$)/.test(filterKey||'');
  RBQ.entries=new Set();
...
function rbqShowCurrent(){
  if(document.getElementById('rbq-end').classList.contains('on'))return; /* 완료창 표시 중엔 진행 갱신 중단 */
  ...
  const prog=document.getElementById('rbq-progress');
  /* 하드코어는 접경국이 몇 개인지 알려 주지 않는다 — 세어서 맞히는 걸 막는다 */
  if(prog){prog.textContent=RBQ.hard?'':'0 / '+RBQ.remaining.size+'개';prog.style.display=RBQ.hard?'none':'';}
  const dots=document.querySelector('#rbq-box .bq-dots');if(dots)dots.style.display=RBQ.hard?'none':'';
  const found=document.getElementById('rbq-found');if(found)found.innerHTML='';
  const qEl=document.getElementById('rbq-q-text');
  if(qEl)qEl.innerHTML=RBQ.hard
    ? '이 나라와 접한 나라를 <b>빠짐없이</b> 입력하고 제출하세요 <span style="font-size:.6rem;color:var(--tx2)">(개수는 알려 주지 않아요 · 하나라도 빠지면 오답)</span>'
    : '이 나라와 접한 나라를 <b>모두</b> 입력하세요 <span style="font-size:.6rem;color:var(--tx2)">(한 나라씩)</span>';
  const tp=document.getElementById('rbq-type');if(tp)tp.style.display='flex';
  const gi0=document.getElementById('rbq-gi');
...
/* 기본 점수는 완벽하게 맞혔을 때만 준다. 하나라도 어긋나면 그 점수는
   통째로 날아가고 깎인 만큼만 남는다 — 그래서 대개 마이너스가 된다. */
function rbqHardPts(iso,missed,extra){
  if(!missed&&!extra)return rbqHardBase(iso);
  return -(RBQ_HARD_MISS*missed+RBQ_HARD_EXTRA*extra);
}
...
function rbqHandleClick(iso){
  ...
  if(RBQ.hard){if(iso!==RBQ.target)rbqHardAdd(iso);return;}
...
/* ── 하드코어: 맞았는지 알려 주지 않고 적어 두기만 한다 ── */
function rbqRenderEntries(){
  const found=document.getElementById('rbq-found');
  if(found){
    found.innerHTML='';
    [...RBQ.entries].forEach(iso=>{
      const sp=document.createElement('span');
      sp.className='bq-nb rbq-entry-tag';
      sp.textContent=(COUNTRIES[iso]?COUNTRIES[iso].k:iso)+' ×';
      sp.title='눌러서 지우기';
      sp.onclick=()=>{RBQ.entries.delete(iso);rbqRenderEntries();};
      found.appendChild(sp);
    });
  }
  const hs=document.getElementById('rbq-hard-submit');
  if(hs){hs.disabled=RBQ.entries.size===0;hs.textContent='제출 ('+RBQ.entries.size+'개 입력)';}
}
function rbqHardAdd(iso){
  if(RBQ.entries.has(iso)){bqFlash('이미 적었어요','bfng');return;}
  RBQ.entries.add(iso);
  rbqRenderEntries();
  const fb=document.getElementById('rbq-fb');
  /* 맞았는지 틀렸는지는 제출할 때까지 말하지 않는다 */
  if(fb){fb.textContent='‘'+(COUNTRIES[iso]?COUNTRIES[iso].k:iso)+'’ 추가';fb.className='bq-fb';}
}
/* 적어 둔 나라가 접경국 전체와 정확히 같아야만 정답이다.
   하나라도 빠지거나 엉뚱한 나라가 섞여 있으면 0점. */
function rbqHardGrade(){
  if(!RBQ.hard||!RBQ.target)return;
  if(!RBQ.entries||!RBQ.entries.size)return;
  if(!RBQ.remaining||!RBQ.remaining.size)return;   /* 이미 채점됨 */
  const tgt=RBQ.target;
  const nbs=(BORDERS[tgt]||[]);
  const hit=nbs.filter(i=>RBQ.entries.has(i));
  const missed=nbs.filter(i=>!RBQ.entries.has(i));
...
function rbqTypeSubmit(){
  ...
  const t=inp.value.trim();if(!t)return;inp.value='';
  if(RBQ.hard){
    const hit=Object.keys(COUNTRIES).find(i=>check(t,i));
    if(hit)rbqHardAdd(hit);
    else{playWrongSound();bqFlash('그런 나라가 없어요','bfng');inp.classList.add('shake');setTimeout(()=>inp.classList.remove('shake'),360);}
    try{inp.focus();}catch(e){}
    return;
  }
...
function rbqSkip(){
  if(RBQ.hard){
    ...
    bqFlash(n>RBQ_HARD_SKIP.length?n+'번 눌렀어요. 그래도 안 돼요':RBQ_HARD_SKIP[n-1],'bfng');
...
function rbqEnd(){
  ...
  el.classList.add('on');
  window._lastResult={title:RBQ.hard?'접경국 하드코어':'접경국 쓰기',score:points+'점',
  ...
  window._pendingReset=()=>resetRBQ(true);
}
...
function rvFlat(r){return r.g.flat();}
/* 세계지도(나라 SVG) 좌표 → 위경도 → 하천SVG 좌표 */
function mainToRv(pt){
  const lon=(pt[0]-RV_MAIN.b)/RV_MAIN.a;
  const lat=RV_MAIN.C[0]+RV_MAIN.C[1]*pt[1]+RV_MAIN.C[2]*Math.pow(pt[1],3);
  return [RV_PROJ.a*lon+RV_PROJ.b, RV_PROJ.C[0]+RV_PROJ.C[1]*lat+RV_PROJ.C[2]*Math.pow(lat,3)];
}                      /* 세그먼트 → 점 목록(회랑 순) */
function rvPathD(r){return r.g.map(sg=>'M'+sg.map(q=>q.join(' ')).join('L')).join('');}
...
function rvStats2(){
  ...
  const rem=document.getElementById('ui-rem'),cor=document.getElementById('ui-cor'),rev=document.getElementById('ui-rev');
  if(rem)rem.textContent=total-done;
  if(cor)cor.textContent=RV.cor;
  if(rev)rev.textContent=RV.wr;
  const pf=document.getElementById('ui-pf');if(pf)pf.style.width=(total?done/total*100:0)+'%';
}
function rvStats(){
  const done=Object.keys(RV.status).length,total=RV.list.length;
...
function rvSetTool(t){
  RV.tool=t;
  const pen=document.getElementById('rvd-pen'),hand=document.getElementById('rvd-hand');
  if(pen)pen.classList.toggle('on',t==='pen');
  if(hand)hand.classList.toggle('on',t==='hand');
  const ov=document.getElementById('draw-ov');
  if(ov)ov.classList.toggle('pen',t==='pen'); /* 이동 모드면 포인터가 지도로 통과 */
}
function rvApplyVB(){
  const svg=document.getElementById('rv-svg');if(!svg||!RV.vb)return;
  const v=RV.vb;
  svg.setAttribute('viewBox',v.x+' '+v.y+' '+v.w+' '+v.h);
  const k=Math.max(.2,v.w/RV_W);
  const q=document.getElementById('rv-river-q'),a=document.getElementById('rv-river-ans'),u=document.getElementById('rv-user'),n=document.getElementById('rv-net');
  if(q)q.style.strokeWidth=(3.2*k*3).toFixed(2);
  if(a)a.style.strokeWidth=(3.2*k*3).toFixed(2);
  if(u)u.style.strokeWidth=(2.6*k*3).toFixed(2);
  if(n)n.style.strokeWidth=Math.min(1.2,(0.5*k*3)).toFixed(2);
}
function rvZoomAt(px,py,f){
  const v=RV.vb;if(!v)return;
  const nw=Math.max(40,Math.min(RV_W,v.w*f)),nh=nw*(v.h/v.w);
  v.x=px-(px-v.x)*(nw/v.w);v.y=py-(py-v.y)*(nh/v.h);
  v.w=nw;v.h=nh;
  v.x=Math.max(-nw*.2,Math.min(RV_W-nw*.8,v.x));v.y=Math.max(-nh*.2,Math.min(RV_H-nh*.8,v.y));
  rvApplyVB();
}
function _rvPt(svg,e){const ctm=svg.getScreenCTM();return new DOMPoint(e.clientX,e.clientY).matrixTransform(ctm.inverse());}
function rvSvg(){
  ...
  mp.innerHTML='<svg id="rv-svg" viewBox="0 0 '+RV_W+' '+RV_H+'" preserveAspectRatio="xMidYMid meet">'
    ...
    +'<path id="rv-net" d="'+RV_NET+'"/>'
    +'<path id="rv-river-q" d=""/>'
    +'<path id="rv-river-ans" d=""/>'
    +'<polyline id="rv-user" points=""/>'
    +'</svg>';
  svg=document.getElementById('rv-svg');
  /* 포인터: 펜(그리기) / 손(이동) / 두 손가락 핀치 줌 / 휠 줌 */
  ...
  svg.addEventListener('pointerdown',e=>{
    ...
    if(ptrs.size===2){
      RV.drawing=false;
      const a=[...ptrs.values()];
      pinch={d:Math.hypot(a[0].x-a[1].x,a[0].y-a[1].y),vb:{...RV.vb}};
      panStart=null;
    }else if(RV.diff==='H'&&RV.tool==='pen'&&!RV._reveal){
      RV.drawing=true;RV.dpts=[];rvAddPt(e);
    }else{
      panStart={cx:e.clientX,cy:e.clientY,vb:{...RV.vb}};
    }
    e.preventDefault();
  });
  svg.addEventListener('pointermove',e=>{
    ...
    if(pinch&&ptrs.size===2){
      ...
      const f=Math.max(.2,Math.min(5,pinch.d/Math.max(1,d)));
      const mx=pinch.vb.x+pinch.vb.w/2,my=pinch.vb.y+pinch.vb.h/2;
      RV.vb={...pinch.vb};rvZoomAt(mx,my,f);
    }else if(RV.drawing){rvAddPt(e);}
    else if(panStart){
      const r=svg.getBoundingClientRect();
      const kx=RV.vb.w/r.width,ky=RV.vb.h/r.height;
      RV.vb.x=panStart.vb.x-(e.clientX-panStart.cx)*kx;
      RV.vb.y=panStart.vb.y-(e.clientY-panStart.cy)*ky;
      rvApplyVB();
    }
  });
  const up=e=>{
    ...
    if(!ptrs.size){RV.drawing=false;panStart=null;}
  };
  svg.addEventListener('pointerup',up);svg.addEventListener('pointercancel',up);
  svg.addEventListener('wheel',e=>{
    e.preventDefault();
    const pt=_rvPt(svg,e);
    rvZoomAt(pt.x,pt.y,e.deltaY>0?1.18:0.85);
  },{passive:false});
  return svg;
}
function rvAddPt(e){
...
function rvShow(){
  ...
  /* 하: 다른 하천은 모두 가리고 타깃만 — 전세계 뷰에서 시작(사전 확대 없음), 직접 찾아 확대 */
  document.getElementById('rv-land').style.display='';
  document.getElementById('rv-net').style.display='none';
  document.getElementById('rv-river-ans').setAttribute('d','');
  const q=document.getElementById('rv-river-q');
  q.setAttribute('d',rvPathD(r));q.style.display='';
  RV.vb={x:0,y:0,w:RV_W,h:RV_H};rvApplyVB();
  qEl.innerHTML='지도에 <b>빨갛게 표시된 물줄기</b>는 어떤 하천일까요? <span style="font-size:.62rem;color:var(--tx2)">(드래그·휠로 탐색)</span>';
...
function rvdStart(){
  ...
  if(ov&&!ov._bound){
    ...
    ov.addEventListener('pointerdown',e=>{
      if(mapMode!=='rvd'||RV.tool!=='pen'||RV._reveal)return;
      RV.drawing=true;RV.dpts=[];ov.setPointerCapture(e.pointerId);
      rvdAddPt(e);e.preventDefault();
    });
    ov.addEventListener('pointermove',e=>{if(RV.drawing&&mapMode==='rvd')rvdAddPt(e);});
    const up=()=>{RV.drawing=false;};
    ov.addEventListener('pointerup',up);ov.addEventListener('pointercancel',up);
  }
  const loop=()=>{ if(mapMode!=='rvd'){_rvdRAF=null;return;} rvdRender(); _rvdRAF=requestAnimationFrame(loop); };
  _rvdRAF=requestAnimationFrame(loop);
}
function rvdStop(){if(_rvdRAF){cancelAnimationFrame(_rvdRAF);_rvdRAF=null;}const ln=document.getElementById('rvd-line');if(ln)ln.setAttribute('points','');}
function rvdAddPt(e){
  const mw=document.getElementById('ui-map');const rc=mw.getBoundingClientRect();
  const mx=(e.clientX-rc.left-_x)/_s, my=(e.clientY-rc.top-_y)/_s; /* 지도 좌표 */
  const last=RV.dpts[RV.dpts.length-1];
  if(last&&Math.hypot(mx-last[0],my-last[1])*_s<3)return;
  RV.dpts.push([mx,my]);
}
function rvdRender(){
  const ln=document.getElementById('rvd-line');if(!ln)return;
  ln.setAttribute('points',RV.dpts.map(p=>(p[0]*_s+_x).toFixed(1)+','+(p[1]*_s+_y).toFixed(1)).join(' '));
}
/* ── 하: 이름 판정 ── */
...
function rvTypeSubmit(){
  ...
  if(rvNameOk(t,RV.cur)){if(fb){fb.textContent='정답! '+RV.cur.ko;fb.className='bq-fb ok';}rvResolve(true,rvPer());return;}
  /* 다른 하천 이름인지 / 존재하지 않는 이름인지 구분 피드백 */
  const other=RIVERS.find(rr=>rvNameOk(t,rr));
  if(!other){
    if(fb){fb.textContent='그런 하천은 없어요';fb.className='bq-fb ng';}
    gi.classList.add('shake');setTimeout(()=>gi.classList.remove('shake'),360);
    return; /* 기회 소모 없음 */
  }
  RV.tries++;rvSetDots(RV.tries);
  if(RV.tries>=3){rvResolve(false,0);}
  else if(fb){fb.textContent=other.ko+' — 아니에요 (기회 '+(3-RV.tries)+')';fb.className='bq-fb ng';}
}
/* ── 상: 그리기 판정 ── */
function rvClearDraw(){RV.dpts=[];document.getElementById('rv-user').setAttribute('points','');const ln=document.getElementById('rvd-line');if(ln)ln.setAttribute('points','');}
...
function rvJudge(){
  ...
  const fb=document.getElementById('rvd-fb');
  if(RV.dpts.length<3){if(fb){fb.textContent='경로를 먼저 그려주세요';fb.className='bq-fb ng';}return;}
  const user=RV.dpts.map(mainToRv); /* 세계지도→하천SVG 좌표 */
  RV._userRv=user;
  const sim=rvSimilarity(user,rvFlat(RV.cur));
...
function rvResolve(ok,pts,band){
  ...
  rvSvg();
  document.getElementById('rv-land').style.display='';
  document.getElementById('rv-net').style.display='';
  document.getElementById('rv-river-q').setAttribute('d','');
  const ans=document.getElementById('rv-river-ans');
  ans.setAttribute('d',rvPathD(r));
  const uEl=document.getElementById('rv-user');
  if(RV.diff==='H'&&RV._userRv&&RV._userRv.length>1){uEl.setAttribute('points',RV._userRv.map(q2=>q2.join(',')).join(' '));}
  else uEl.setAttribute('points','');
  rvFit(rvBbox(rvFlat(r)),2.4);
  if(RV.diff==='H'){document.getElementById('rv-screen').classList.add('on');rvStats();}
...
function rvNext(){
  ...
  if(RV.diff==='H'){document.getElementById('rv-screen').classList.remove('on');rvClearDraw();}
  if(mapMode==='rvc'||mapMode==='rvd'){clearMapColors();paint();}
  rvStats2();
  rvShow();
}
function rvSkip(){
...
function submit(){
  ...
  if(check(val,iso)){
    ...
    document.getElementById('ui-fb').textContent='틀렸어요';document.getElementById('ui-fb').className='fb ng';
    if(w<2){
      playWrongSound();
...
function retryWrong(minOverride){
  ...
  window._pendingReset=null; /* 오답 재도전이 리셋을 대체 */
  document.getElementById('ui-end').style.display='none';
...
function startOopsQuiz(){
  ...
  if(!saved.length){alert('저장된 오답이 없어요.');return;}
  document.getElementById('ui-end').style.display='none';
...
function endScreen(){
  ...
  window._pendingReset=()=>resetMapQuiz(true); /* 결과창 닫을 때 자동 리셋 */
}
...
function renderCLGrid(){
  ...
  if(clTab==='done'){
    ...
    if(!sorted.length){grid.innerHTML='<span style="color:var(--tx2);font-size:.8rem">아직 맞춘 나라가 없어요.</span>';}
  }else if(clTab==='wrong'){
    const pool=S.activeSet?[...S.activeSet]:Object.keys(COUNTRIES);
    ...
    if(!wrongISOs.length){grid.innerHTML='<span style="color:#188038;font-size:.8rem">틀린 나라가 없어요</span>';}
  }else{
    const pool=S.activeSet?[...S.activeSet]:Object.keys(COUNTRIES);
    ...
    if(!remain.length){grid.innerHTML='<span style="color:#188038;font-size:.8rem">모든 나라를 맞혔어요</span>';}
  }
}
...
function checkRQ(){
  ...
  if(allOk){
    ...
    /* 색상은 reset */
    clearRQInputs();
    document.getElementById('rq-fb').textContent='틀렸어요 · 한 번 더!';document.getElementById('rq-fb').className='rq-fb ng';
    return;
  }else{
    RQ.errors++;
    ...
    document.getElementById('rq-fb').textContent='틀렸어요';document.getElementById('rq-fb').className='rq-fb ng';
    document.getElementById('rq-pts-earned').textContent='+'+earned+'pt / '+maxPts+'pt';
...
function krSubmit(){
  ...
  if(krCheckAnswer(val,kid)){
    ...
    KR.wrong[kid]=w+1;
    gi.classList.add('shake');setTimeout(()=>gi.classList.remove('shake'),400);
    document.getElementById('kr-fb').textContent='틀렸어요';document.getElementById('kr-fb').className='fb ng';
    if(w<2){
      playWrongSound();
...
function renderKLGrid(){
  ...
  if(klTab==='done'){
    ...
    if(!sorted.length)grid.innerHTML='<span style="color:var(--tx2);font-size:.8rem">아직 맞춘 지역이 없어요.</span>';
  }else{
...
function initMap(){
  ...
  svg.addEventListener('click',function(e){
    ...
    if(mapMode==='suteuk'){try{sqMapClick(iso);}catch(e){}return;}
  ...
  svg.addEventListener('mouseover',function(e){
    ...
    /* 수특퀴즈 지도 문항은 나라 이름 자체가 정답이라 툴팁을 띄우지 않는다 */
    if(mapMode==='rvd'||mapMode==='climate'||mapMode==='suteuk'){tip.style.display='none';return;}
...
function listSaves(){
  ...
  for(let i=0;i<localStorage.length;i++){
    ...
    }else if(/^sq_(all|c[a-z-]+(_f[a-z-]+)?|f[a-z-]+)$/.test(k)){
...
const CHAP_NARR_X={
  527:'석유류(원유·정유제품)가 수출의 큰 축이에요. 유전을 가진 산유국이거나, 원유를 수입해 정제·재수출하는 정유 산업이 발달한 나라예요.',
  526:'철광석·구리 등 광물을 캐서 파는 자원 부국이에요. 식민지 시기부터 광산이 개발돼 원자재 수출 구조가 굳어졌고, 가공보다 원광 형태로 많이 내보내요.',
  1471:'금·다이아몬드 등 귀금속이 수출의 큰 축이에요. 광산 자원을 가졌거나, 자본·기술을 바탕으로 귀금속을 들여와 가공하거나 중계하기도 해요.',
  631:'인광석·천연가스를 바탕으로 비료를 만들어 파는 자원·화학 경제예요. 풍부한 광물 자원이 비료 산업으로 이어졌어요.',
  1685:'전자·기계 등 공산품 수출이 중심인 제조 강국이에요. 전후 수출주도 산업화로 외국 자본·공장을 끌어들여 제조업을 키웠고, 부가가치 높은 완제품을 세계에 팝니다.',
  1684:'기계·설비 등 공산품 수출이 중심인 제조 선진국이에요. 오랜 산업화로 축적한 기술을 바탕으로 고부가 완제품을 수출해요.',
  1787:'자동차·부품 수출이 중심인 제조 강국이에요. 자동차 산업을 국가 전략으로 키워 완성차와 부품을 세계에 수출해요.',
  ...
  418:'카카오 등 기호작물 수출 비중이 큰 열대 농업 경제예요. 식민지 시대 플랜테이션의 흔적이 남아 있어요.',
  ...
  628:'알루미나 등 광물 가공품 수출 비중이 큰 자원 경제예요. 보크사이트 등 광물을 가공해 수출해요.',
...
const CHAP_NARR_M={
  ...
  1787:'자동차를 수입에 의존하는 나라예요. 자국 자동차 산업이 거의 없어요.',
