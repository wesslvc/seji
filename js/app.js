...
/* ── 육지 접경(인접국) 데이터 — 접경국 맞추기 퀴즈용 ──
   [단위 정책] 접경국 퀴즈에서 정답/클릭/깜빡임 대상은 COUNTRIES에 등록된 198개국 +
   예외 4곳(대만 tw · 팔레스타인 ps · 코소보 xk · 서사하라 eh)뿐이다. 이 4곳은
   COUNTRIES에도 개별 등록돼 있고 지도(world-svg)에서도 본국과 분리된 영역으로
   클릭·깜빡임이 가능하다(대만은 육지 접경이 없어 이 표엔 등장하지 않음).
   그 외 미승인국·분쟁지역 — 소말릴란드(→소말리아 so), 압하지야·남오세티야
   (→조지아 ge), 나고르노카라바흐(→아제르바이잔 az), 트란스니스트리아(→몰도바 md),
   도네츠크·루한스크(→우크라이나 ua), 북키프로스(→키프로스 cy) 등 — 는 의도적으로
   COUNTRIES/BORDERS에 별도 등재하지 않는다. 지도상 경계선도 안 그려서(css의
   .limitxx/.unxx 기본값 = 투명·클릭불가) 본국과 하나로 보이고, 클릭·깜빡임·정답
   판정도 전부 본국 기준으로만 동작한다. 새 분쟁지역이 생겨도 이 정책을 따를 것 —
   정말 별도로 맞혀야 하는 예외가 아니면 COUNTRIES/BORDERS에 추가하지 말 것. */
const BORDERS={
